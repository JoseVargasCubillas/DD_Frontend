...
interface NewOfferModalProps {
  courses: Course[];
  onClose: () => void;
  onCreate: (offer: OfferRecord) => void;
}

function NewOfferModal({ courses, onClose, onCreate }: NewOfferModalProps) {
  ...
  const filtered = courses.filter((c) =>
    c.title.toLowerCase().includes(searchProduct.toLowerCase()),
  );
  ...
  const handleCreate = () => {
    ...
    const courseId = primaryCourse
      ? String(primaryCourse._id || primaryCourse.id || primaryCourse.slug || "")
      : "standalone";
    const offer: OfferRecord = {
      ...
      price: paymentType === "free" ? 0 : Number(price) || 0,
      currency: "MXN",
      paymentType: paymentType === "subscription" ? "one_time" : paymentType,
      thumbnail: primaryCourse?.thumbnail || "",
      courseTitle: primaryCourse?.title || title.trim(),
      checkoutPath: "/checkout?offer=" + encodeURIComponent(title.trim()),
    };
    onCreate(offer);
  };

  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-ink-900/50 px-4 py-10">
      <div className="flex w-full max-w-5xl rounded-2xl bg-white shadow-2xl overflow-hidden">
        {/* Left: form */}
        <div className="flex-1 p-8">
          <div className="flex items-center justify-between mb-6">
            <h2 className="font-serif text-2xl text-ink-900">Nueva oferta</h2>
            <button
              type="button"
              onClick={onClose}
              className="h-8 w-8 rounded-full text-ink-500 hover:bg-ink-900/6 flex items-center justify-center text-xl"
            >
              &times;
            </button>
          </div>
          ...
          <p className="text-base font-semibold text-ink-900 mb-1">
            Que producto(s) estas vendiendo?
          </p>
          <p className="text-sm text-ink-500 mb-6">
            Vender nunca ha sido mas facil. Te ayudaremos a fijar el precio y vender tu oferta.
          </p>

          {/* Offer title */}
          <label className="block mb-5">
            <span className="text-sm font-semibold text-ink-700">Titulo de la oferta</span>
            <input
              value={title}
              ...
              className={
                "mt-1.5 block w-full rounded-lg border px-3 py-2.5 text-sm outline-none focus:border-ink-900 " +
            ...
            />
            {titleTouched && !title.trim() && (
              <span className="text-xs text-red-500 mt-1 block">El titulo no puede estar en blanco</span>
            )}
          </label>

          {/* Product selector */}
          <div className="block mb-6">
            <span className="text-sm font-semibold text-ink-700">Elegir productos a vender</span>
            <div className="relative mt-1.5">
              <button
                type="button"
                onClick={() => setDropdownOpen((o) => !o)}
                className={
                  "flex w-full items-center justify-between rounded-lg border px-3 py-2.5 text-sm text-ink-700 hover:border-ink-900 " +
                  (productsTouched && selectedIds.length === 0 ? "border-red-400 bg-red-50" : "border-ink-900/20")
                }
              >
              ...
              {dropdownOpen && (
                <div className="absolute z-10 mt-1 w-full rounded-xl border border-ink-900/15 bg-white shadow-xl">
                  ...
                  <div className="max-h-52 overflow-y-auto py-1">
                    {filtered.map((c) => {
                      ...
                      return (
                        <label
                          key={id}
                          className="flex cursor-pointer items-center gap-3 px-3 py-2 text-sm hover:bg-ink-900/5"
                        >
                          <input
                            type="checkbox"
                            checked={checked}
          ...
          {/* Payment type */}
          <div className="mb-5">
            <span className="text-sm font-semibold text-ink-700 block mb-2">Tipo de pago</span>
            <div className="flex gap-3">
              {[
                ["free", "Gratis"],
                ["one_time", "Pago unico"],
                ...
                <button
                  ...
                  className={
                    ...
                    (paymentType === value
                      ...
                      : "border-ink-900/20 text-ink-600 hover:border-ink-900")
                  }
                >
                  {label}
                </button>
          ...
          {paymentType !== "free" && (
            <label className="block mb-6">
              <span className="text-sm font-semibold text-ink-700">
                Monto {paymentType === "subscription" ? "mensual" : ""}
              </span>
              <div className="mt-1.5 flex items-center gap-2">
                <input
                  ...
                  placeholder="0.00"
                  className="w-40 rounded-lg border border-ink-900/20 px-3 py-2.5 text-sm outline-none focus:border-ink-900"
                />
                <span className="text-sm text-ink-500">MXN</span>
              </div>
            </label>
          )}

          <div className="flex items-center justify-end gap-3 pt-4 border-t border-ink-900/10">
            <button
              type="button"
              onClick={onClose}
              className="rounded-full border border-ink-900/20 px-5 py-2 text-sm font-medium hover:border-ink-900"
            >
              Cancelar
            </button>
            <button
        ...
        <div className="w-80 shrink-0 bg-gray-50 flex items-center justify-center p-6 border-l border-ink-900/10">
          <div className="w-full rounded-2xl bg-white border border-gray-200 shadow-sm p-6">
            <p className="text-center text-2xl font-bold text-ink-900 mb-5">{displayPrice}</p>
            <div className="space-y-3">
              <div>
                <label className="text-xs text-ink-500">Contacto</label>
                <div className="mt-1 h-9 rounded-lg border border-gray-200 bg-gray-50 px-3 flex items-center text-sm text-gray-400">
                  Correo electronico
                </div>
              </div>
              {paymentType !== "free" && (
                <>
                  <div>
                    <label className="text-xs text-ink-500">Nombre completo</label>
                    <div className="mt-1 h-9 rounded-lg border border-gray-200 bg-gray-50 px-3 flex items-center text-sm text-gray-400">
                      Nombre y apellido
                    </div>
                  </div>
                  <div>
                    <label className="text-xs text-ink-500">Numero de tarjeta</label>
                    <div className="mt-1 h-9 rounded-lg border border-gray-200 bg-gray-50 px-3 flex items-center justify-between text-sm text-gray-400">
                      Numero de tarjeta
                      <span className="flex gap-1 text-xs text-gray-300">VISA MC</span>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <div className="flex-1 h-9 rounded-lg border border-gray-200 bg-gray-50 px-3 flex items-center text-sm text-gray-400">
                      MM/AA
                    </div>
                    <div className="w-20 h-9 rounded-lg border border-gray-200 bg-gray-50 px-3 flex items-center text-sm text-gray-400">
                      CVC
                    </div>
                  </div>
                </>
              ...
              <button
                type="button"
                className="mt-2 w-full rounded-full bg-ink-900 py-2.5 text-sm font-semibold text-white"
              >
                {paymentType === "free" ? "Obtener gratis" : "Pagar " + displayPrice}
              </button>
